const { getCart, createCart, updateCart } = require("../Services/cartServices");
const client = require("../config/dbconfig");

module.exports = {
  createCart: async (req, res) => {
    const body = req.body;
    // console.log(body.product.product.user_id +"userid"+body.product.product.id+"product_id"+  body.product.product.price+"price");
    ...
    name:productInfo.rows[0].name,image:productInfo.rows[0].image,price:productInfo.rows[0].price});
  ...
  getCart:async(req,res)=>{
    ...
    const cartList = await client.query(
      `SELECT cart.customer_id,cart.product_id,cart.count,product.name,product.image,product.price FROM cart INNER join product ON product.id = cart.product_id where customer_id=$1`,
      [body.id]
    );
